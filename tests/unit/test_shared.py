...
from shared.schemas.events import OrderCreatedEvent, OrderItemPayload, PaymentCompletedEvent
...
def test_sensitive_data_sanitization():
    raw_payload = {
        ...
        "items": [
            {"product_id": "prod_1", "card_number": "1234"}
        ]
    }
...
async def test_event_bus_in_memory_fallback():
    ...
    sample_order_event = OrderCreatedEvent(
        ...
        total_amount=150.00,
        currency="USD",
        items=[OrderItemPayload(product_id="prod_1", quantity=2, unit_price=75.00, subtotal=150.00)],
    )
